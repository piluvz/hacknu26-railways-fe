...
const STATUS_CONFIG: Record<NodeStatus, { color: string; bg: string; border: string; label: string }> = {
    ok:      { color: "#49C86E", bg: "#49C86E1A", border: "#49C86E", label: "Норма"    },
    warning: { color: "#EABD52", bg: "#EABD521A", border: "#EABD52", label: "Внимание" },
    error:   { color: "#E23F3F", bg: "#E23F3F1A", border: "#E23F3F", label: "Критично" },
};

const ALERT_NODES = NODES_FROM_BACKEND
...
function NodeCard({ node, index }: { node: NodeData; index: number }) {
    ...
    return (
        ...
        >
            <span className="text-xs font-regular text-white break-words">{node.label}</span>
...
export default function StatesWidget() {
    ...
    return (
        <div className="flex flex-col px-5 pt-4 pb-5 bg-[#171719] text-white border border-[#222223] rounded-xl h-full">
            <span className="text-sm text-[#696969] uppercase tracking-[0.08em] mb-3">Состояние узлов</span>
